 ...
 * JASSUB v2.x requires WebGPU for hardware-accelerated rendering.
 */
...
/**
 * Applies overlay positioning to the JASSUB wrapper element.
 * JASSUB creates a wrapper div with class "JASSUB" as a sibling to the video.
 *
 * @param videoElement - The video element JASSUB is attached to
 */
export function applyJassubOverlayStyles(videoElement: HTMLVideoElement): void {
  const wrapper = videoElement.nextElementSibling as HTMLElement | null

  if (!wrapper?.classList.contains('JASSUB')) {
    // Wrapper not found - this can happen during initialization
    return
  }
